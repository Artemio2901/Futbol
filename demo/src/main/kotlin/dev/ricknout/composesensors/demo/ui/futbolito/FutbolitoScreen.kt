    ...
    Scaffold(
        ...
        Box(
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                ...
                // Walls (Obstacles)
                val wallT = 16.dp.toPx()
                val w = size.width
                val h = size.height
                val walls = listOf(
                    Rect(w * 0.15f, h * 0.2f, w * 0.4f, h * 0.2f + wallT),
                    Rect(w * 0.6f, h * 0.2f, w * 0.85f, h * 0.2f + wallT),
                    Rect(w * 0.15f, h * 0.8f, w * 0.4f, h * 0.8f + wallT),
                    Rect(w * 0.6f, h * 0.8f, w * 0.85f, h * 0.8f + wallT),
                    Rect(w * 0.25f, h * 0.4f, w * 0.25f + wallT, h * 0.6f),
                    Rect(w * 0.75f - wallT, h * 0.4f, w * 0.75f, h * 0.6f),
                    Rect(w * 0.42f, h * 0.35f, w * 0.58f, h * 0.35f + wallT),
                    Rect(w * 0.42f, h * 0.65f, w * 0.58f, h * 0.65f + wallT),
                    Rect(w * 0.08f, h * 0.45f, w * 0.08f + wallT, h * 0.55f),
                    Rect(w * 0.92f - wallT, h * 0.45f, w * 0.92f, h * 0.55f)
                )

                walls.forEach { wall ->
                    val corner = CornerRadius(wallT / 2)
                    drawRoundRect(
                        color = BallShadow,
                        topLeft = Offset(wall.left + 4f, wall.top + 4f),
                        size = wall.size,
                        cornerRadius = corner
                    )
                    drawRoundRect(
                    ...
                    drawRoundRect(
                        ...
                        topLeft = Offset(wall.left + 2f, wall.top + 2f),
                        size = Size(wall.width - 4f, wall.height - 4f),
                        cornerRadius = corner,
            ...
            LaunchedEffect(Unit) {
                ...
                while (true) {
                    withFrameMillis { frameMs ->
                        ...
                        // Inverted controls based on user feedback
                        ...
                        val wallT = ballRad * (16f / BALL_RADIUS_DP)
                        val walls = listOf(
                            Rect(w * 0.15f, h * 0.2f, w * 0.4f, h * 0.2f + wallT),
                            Rect(w * 0.6f, h * 0.2f, w * 0.85f, h * 0.2f + wallT),
                            Rect(w * 0.15f, h * 0.8f, w * 0.4f, h * 0.8f + wallT),
                        ...
                        for (wall in walls) {
                            val closestX = nx.coerceIn(wall.left, wall.right)
                            val closestY = ny.coerceIn(wall.top, wall.bottom)

                            val dx = nx - closestX
                            val dy = ny - closestY
                            val distSq = dx * dx + dy * dy

                            if (distSq < ballRad * ballRad) {
                                val dist = java.lang.Math.sqrt(distSq.toDouble()).toFloat()
                                if (dist > 0f) {
                                    val nX = dx / dist
                                    val nY = dy / dist
                                    val overlap = ballRad - dist
                                    nx += nX * overlap
                                    ny += nY * overlap
                                    ...
                                    ny -= ballRad
                                    vy = -(if (vy < 0f) -vy else vy) * BOUNCE
                                }
                            }
                        }

                        // ── Wall collisions ───────────────────────────────────────────
                        if (nx - ballRad < 0f)   { nx = ballRad;         vx = (if (vx < 0f) -vx else vx) * BOUNCE }
                        if (nx + ballRad > w)     { nx = w - ballRad;    vx = -(if (vx < 0f) -vx else vx) * BOUNCE }
